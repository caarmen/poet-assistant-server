...
class WotdServiceTest {
    ...
    private fun assertWotd(dateString: String, expectedWotd: String) {
        val date = LocalDate.parse(dateString)
        val actualWotdList = controller.getWotd(date, 1)
        assertEquals(1, actualWotdList.size)
        assertEquals(date, actualWotdList.first().date)
        assertEquals(expectedWotd, actualWotdList.first().word)
    }

    @Test
    fun testListWotd() {
        val date = LocalDate.parse("2021-11-28")
        val expectedWotdList = listOf(
        ...
        val actualWotdList = controller.getWotd(date, 3)
