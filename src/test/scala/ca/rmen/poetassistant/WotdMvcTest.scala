...
import ca.rmen.poetassistant.api.WotdController.{QUERY_PARAM_BEFORE, QUERY_PARAM_SIZE, SERVICE}
...
class WotdMvcTest {
   ...
   */
  @Test
  def testWotdSpecificDateListOk(): Unit = {
    mockMvc.perform(get(s"$SERVICE?$QUERY_PARAM_BEFORE=2021-11-22&$QUERY_PARAM_SIZE=3"))
      .andExpect(status().isOk)
  ...
  @Test
  def testWotdSpecificDateOneItemOk(): Unit = {
    mockMvc.perform(get(s"$SERVICE?$QUERY_PARAM_BEFORE=2021-11-21"))
      .andExpect(status().isOk)
      .andExpect(content().string(containsString("teem")))
  }

  @Test
  def testWordTooBigPage(): Unit = {
    mockMvc.perform(get(s"${SERVICE}?${QUERY_PARAM_SIZE}=500"))
      .andExpect(status().isBadRequest)
  }

  @Test
  def testWordZeroSizePage(): Unit = {
    mockMvc.perform(get(s"${SERVICE}?${QUERY_PARAM_SIZE}=0"))
      .andExpect(status().isBadRequest)
  }

  @Test
  def testWordNegativeSizePage(): Unit = {
    mockMvc.perform(get(s"${SERVICE}?${QUERY_PARAM_SIZE}=-5"))
      .andExpect(status().isBadRequest)
  }
}
