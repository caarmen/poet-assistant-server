...
@SpringBootTest
public class WotdServiceTest {

    @Autowired
    private WotdController controller;
    ...
    private void assertWotd(String dateString, String expectedWotd) {
        var date = LocalDate.parse(dateString);
        var actualWotdList = controller.getWotd(date, 1);
        assertEquals(1, actualWotdList.size());
        assertEquals(date, actualWotdList.get(0).date());
        assertEquals(expectedWotd, actualWotdList.get(0).word());
    }

    @Test
    void testListWotd() {
        var date = LocalDate.parse("2021-11-28");
        var expectedWotdList = List.of(
                new WotdApiModel(
                        LocalDate.parse("2021-11-28"),
                        "clothesline"
                ),
                new WotdApiModel(LocalDate.parse("2021-11-27"), "solstice"),
                new WotdApiModel(
                        LocalDate.parse("2021-11-26"),
                        "methylphenidate"
                )
        );
        var actualWotdList = controller.getWotd(date, 3);
