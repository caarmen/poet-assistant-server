...
public class WotdService {
    ...
    private static final int MIN_INTERESTING_FREQUENCY = 1501;
    private static final int MAX_INTERESTING_FREQUENCY = 24999;
    ...
    // request
    private final LazyField<List<StemEntity>> interestingWords = new LazyField<>(this::readInterestingWords);

    public WotdService(StemRepository repository) {
        this.repository = repository;
    }

    private List<StemEntity> readInterestingWords() {
        return repository.findByGoogleNgramFrequencyBetween(MIN_INTERESTING_FREQUENCY, MAX_INTERESTING_FREQUENCY);
    }

    public List<WotdServiceModel> findWotdEntries(LocalDate before, int size) {
        return IntStream.range(0, size)
                .mapToObj((resultPosition) -> {
                    var dateForWotd = before.minusDays(resultPosition);
                    var seed = dateForWotd.atStartOfDay(ZoneOffset.UTC).toInstant().toEpochMilli();
                    var positionForDate = new Random(seed)
                            .nextInt(interestingWords.get().size());
                    return new WotdServiceModel(
                            dateForWotd,
                            interestingWords.get().get(positionForDate).word
                    );
                })
                .toList();
    }
